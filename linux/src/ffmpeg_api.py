...
class FfmpegAPI:
	...
	def create_sprites_for_sheet(self, duration: str, video_file_path: str, burn):
		for i in range(1, 57):
			...
			if burn:
				...
				tmp_image_path = os.path.join(CACHE_PATH, f'image{i:02d}.png').replace("\\", '/')
	...
	def combine_sprites_into_sheet(self, output_image_path: str):
		command = f"ffmpeg -y -i \"{CACHE_PATH}/image%02d.png\" -vf \"scale=317:-1,tile=8x7:color=0x333333:margin=2:padding=2,scale=2560:-1\" -q:v 3 \"{output_image_path}\""
		process = subprocess.Popen(command, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
		ffmpeg_output, ffmpeg_error = process.communicate()
